...
def train(model, graphlist, num_epoch):
    ...
    for epoch in range(num_epoch):
        ...
        logits = logits.float()
        train_labels = np.zeros(shape=len(labels))
        for i in range(len(labels)):
            if labels[i] == 1:
                train_labels[i] = np.array([1])
            else:
...
def test(model, ttH, tt):
    ...
    # tesing phase:
    ttH_test_glist = prep_test_graphlist(ttH, True, 4000, 15000)
    tt_test_glist = prep_test_graphlist(tt, False, 1500, 5000)
    true_pos = 0
    tot_pos = 0
    true_neg = 0
    tot_neg = 0
    for i in range(4000):
        ...
        logits = logits.float()
        loss1 = F.cross_entropy(logits, torch.tensor([1]))
        loss2 = F.cross_entropy(logits, torch.tensor([0]))
...
def main():
    ...
    model = GIN(3, 3, 5, 20, 5, 0.2, False, "mean", "mean")
    test(model, ttH, tt)
    ...
    for i in range(1000):
        ...
        tt_train = prep_train_graphlist(tt, False, 5000)
        all_list = ttH_train + tt_train
        model = train(model, all_list, 100)
